    ...
    <div style={{ padding: 20, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <section style={{ marginBottom: 20 }}>
        ...
        <BuddyCanvas
          ...
          width={300}
          height={300}
          onTap={() => console.log('Buddy clicked!')}
      ...
      <section>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => buddyRef.current?.triggerBlink()}>Blink</button>
        </div>
      </section>
    </div>
  );
...
- **Triggers** - Fire once to transition (e.g., `wave`, `jump`)
...
const waveTrigger = inputs.find(i => i.name === 'wave');
waveTrigger.fire(); // For triggers
```

### Two-Layer Architecture

The state machine uses two layers for independent animations:

- **BodyLayer**: Handles body movements (idle, wave, jump)
- **BlinkLayer**: Handles eye animations (auto-blinking loop)

This allows blinking to continue while other animations play.
...
- [x] Rive file loads without embedded assets
- [x] Each body part loads from CDN
- [x] Console shows no 404 errors
- [x] Buddy starts in idle animation (looping)
- [x] Automatic blinking occurs every 2-5 seconds
- [x] Click triggers wave animation
- [x] Wave/Jump/Blink buttons work correctly
- [x] Animations return to idle after playing
- [x] Character switch reloads all assets
